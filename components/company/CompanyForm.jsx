...
const CompanyForm = () => {
  ...
  const url =
    "https://bkcsaqsiloxvfsnhymgk.supabase.co/storage/v1/object/public/avatars/";
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      const extension = image.uri.substring(
      ...
      <View className="flex justify-center items-center w-screen p-4">
        ...
        <View className="my-8 w-full">
          <Text className="text-xs text-gray-400">
            Tap to upload logo to be printed on invoices/estimates
          </Text>
          <TouchableOpacity
            className="my-2 py-2 border-[2px] border-gray-700 rounded-lg bg-gray-100"
            onPress={pickImage}
          >
            <Ionicons
